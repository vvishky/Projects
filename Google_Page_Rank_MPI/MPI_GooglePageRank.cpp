...
vector<vector<double>> randMatrix(int n) {
    vector<vector<double>> randomMatrix(n, vector<double>(n));

    srand(100); 

    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            randomMatrix[i][j] = rand()%2; 
        }
    }
...

{
    ...
    n = std::atoi(argv[1]); // get the number from command line for size of matrix
    
    //Broadcast number of web pages
    MPI_Bcast(&n, 1, MPI_INT, 0, MPI_COMM_WORLD);

   
    // Generate the random matrix
    vector<vector<double>> L = randMatrix(n);
    ...
    for (int iter = 0; iter < max_iter; ++iter)
    {
        ...
        // Compute Local L1 norm of q for normalization
    ...
    {   
        ...
        //printMatrix(P,"Stochastic matrix");
